...
import { IGetConsultationBaseVm } from '../../api/generated';
import Spinner from '../common/Spinner';
import { parseDateString } from '../../helpers/utils';

interface Props {
  consults: IGetConsultationBaseVm[];
  patientId: string;
  casefileId: string;
  isFetching: boolean;
}
...
const Consultations = ({ consults, patientId, casefileId, isFetching }: Props) => (
  <div className={style.list}>
    <div className={style.header}>
      <Button type="primary">
        <Link to={`/patients/${patientId}/casefiles/${casefileId}/consultations/new`}>New Consult</Link>
      </Button>
    </div>
    <List bordered>
      {isFetching ? (
        ...
        consults.map((consult) => (
          <Link to={`/patients/${patientId}/casefiles/${casefileId}/consultations/${consult.id}`} key={consult.id}>
