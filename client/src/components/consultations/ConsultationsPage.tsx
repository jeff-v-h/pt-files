...
class ConsultationsPage extends React.Component<Props> {
  ...
  private ensureDataFetched = () => {
    ...
    const cId = parseInt(match.params.casefileId);
    if (list.length === 0 || id !== cId) getConsults(cId);
  };
  ...
  render() {
    ...
    return (
      <>
        <PatientInfo />
        <CasefileInfo />
        <Consultations consults={list} patientId={patientId} casefileId={casefileId} isFetching={isFetching} />
      </>
    );
  }
}
